...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6">{{translate('SYNCSMS Credential')}}</h5>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('update_credentials') }}" method="POST">
                        <input type="hidden" name="otp_method" value="syncsms">
                        @csrf
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="SYNC_API_KEY">
                            <div class="col-lg-3">
                                <label class="col-from-label">{{translate('SYNC_API_KEY')}}</label>
                            </div>
                            <div class="col-lg-6">
                                <input type="text" class="form-control" name="SYNC_API_KEY" value="{{  env('SYNC_API_KEY') }}" placeholder="SYNC_API_KEY" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="SYNC_SLOT">
                            <div class="col-lg-3">
                                <label class="col-from-label">{{translate('SYNC_SLOT')}}</label>
                            </div>
                            <div class="col-lg-6">
                                <input type="text" class="form-control" name="SYNC_SLOT" value="{{  env('SYNC_SLOT') }}" placeholder="SYNC_SLOT" required>
                            </div>
                        </div>
                        <div class="form-group mb-0 text-right">
                            <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
