...
@section('content')
	...
	<section class="content no-print">
		<div class="alert alert-warning">
			<p>Editing will reset payment status of these items</p>
		</div>
		{!! Form::open(['url' => action('CourierAssigmentController@update', ['id'=>$assignment->id]), 'method' => 'put', 'id' => 'stock_transfer_form' ]) !!}
		<div class="box box-solid">
			<div class="box-body">
				<div class="row">
					<div class="col-sm-3">
						<div class="form-group">
							...
							<span class="input-group-addon">
								...
								{!! Form::text('date', @format_datetime($assignment->date), ['class' => 'form-control', 'id'=>'date','readonly', 'required']); !!}
							</div>
						</div>
					</div>
					<div class="col-sm-3">
					...
					<div class="col-sm-3">
						<div class="form-group">
							<input type="hidden" id="courier_id" value="{{$assignment->courier_id}}" name="courier_id">
							{!! Form::label('courier_id_select', __('custom.courier').':*') !!}
							{!! Form::select('courier_id_select', $couriers, $assignment->courier_id, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required', 'id' => 'courier_id_select']); !!}
						</div>
					</div>
				</div>
			</div>
					...
					<div class="col-sm-8 col-sm-offset-2">
						<div class="form-group">
							...
							<span class="input-group-addon">
								...
								{!! Form::text('search_order', null, ['class' => 'form-control', 'id' => 'search_order', 'placeholder' => __('custom.search_order')]); !!}
							</div>
						</div>
					</div>
				</div>
									...
									@foreach($list as $order)
										...
										<tr class="product_row">
											...
											@if(env('EnableRedx') || env('PATHAO_ENABLE'))
													...
													@if($assignment->courier_id == 1 && env('PATHAO_ENABLE'))
														<div>
															<input type="hidden" id="{{$row_index}}_city_value" value="{{$order->city_id}}">
															<div class="form-group">
																{!! Form::select('city_id['.$order->id.']', $cities, $order->city_id, ['class' => 'form-control select2','required'=>$assignment->courier_id == 1?1:0,'id'=>$row_index.'_city', 'placeholder'=>'Select city']); !!}
															</div>
														</div>
													@elseif($assignment->courier_id == 2 && env('EnableRedx'))
														<div class="form-group">
															{!! Form::label('redx_area_id', 'Redx Area:*') !!}
															{!! Form::select('redx_area_id['.$order->id.']', $redx_areas, $order->redx_area_id, ['class' => 'form-control select2','required','id'=>$order->id.'_area', 'placeholder'=>'Select area']); !!}
														</div>
													@endif
											@endif
											</td>
											...
											@if(env('PATHAO_ENABLE'))
													...
													@if($assignment->courier_id == 1)
														<div>
															<input type="hidden" id="{{$row_index}}_zone_value" value="{{$order->zone_id}}">
															<div class="form-group">
																{!! Form::select('zone_id['.$order->id.']', $zones, $order->zone_id, ['class' => 'form-control select2 zone','id'=>$row_index.'_zone','required'=>$assignment->courier_id == 1?1:0,'placeholder'=>'Select city first']); !!}
															</div>
														</div>
													@endif
											@endif
...
{{--	<script src="{{ asset('js/stock_transfer.js?v=' . $asset_v) }}"></script>--}}
	<script>
		...
		var listed={!! json_encode($assignment->order_ids, JSON_NUMERIC_CHECK) !!};
		$('#date').datetimepicker({
			format: moment_date_format + ' ' + moment_time_format,
			ignoreReadonly: true,
		});
		$(document).ready(function() {
			var courier_id = "{{$assignment->courier_id}}"
			if (courier_id == 5){
				toggleMask('show')
			}else {
				toggleMask('hide')
			...
			$('select#courier_id_select').change(function () {
		...
		function update_table_total() {
			if (listed.length>0){
				...
				$('#courier_id_select').prop("disabled", true);
			} else {
				$('#courier_id_select').prop("disabled", false);
			}
			var table_total = 0;
