...
class CategoryUtility
{
    ...
    public static function get_immediate_children_menu($id, $with_trashed = false, $as_array = false)
    {
        $children = $with_trashed ? Category::where('parent_id', 0)->orderBy('order_level', 'ASC')->get() : Category::where('parent_id', 0)->orderBy('order_level', 'ASC')->get();
        $children = $as_array && !is_null($children) ? $children->toArray() : $children;
    ...
    public static function get_immediate_children_ids_menu($id, $with_trashed = false)
    {

        $children = CategoryUtility::get_immediate_children_menu($id, $with_trashed, true);
